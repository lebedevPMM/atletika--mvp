...
const CommunityScreen: React.FC<CommunityScreenProps> = ({ onNavigate }) => {
    ...
    return (
        <div
            ...
            style={{
                backgroundColor: isNeon ? 'var(--brand-bg-primary)' : undefined,
                color: isNeon ? 'var(--brand-text-primary)' : undefined
            }}
        >
            {!isNeon && <div className={`absolute inset-0 ${isDark ? 'bg-zinc-950' : 'bg-gray-50'} -z-10`} />}
            ...
            >
                {!isNeon && <div className={`absolute inset-0 ${isDark ? 'bg-zinc-950/80' : 'bg-white/80'} border-b ${isDark ? 'border-zinc-800' : 'border-gray-200'} -z-10`} />}
                <h1
                ...
                >
                    {!isNeon && <div className={`absolute inset-0 rounded-xl ${isDark ? 'bg-zinc-900' : 'bg-gray-100'} -z-10`} />}
                    <button
                    ...
                    >
                        {!isNeon && <span className={activeTab === 'active' ? (isDark ? 'bg-zinc-800 text-white shadow-sm' : 'bg-white text-black shadow-sm') : 'text-gray-400'}></span>}
