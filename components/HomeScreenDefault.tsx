...
export const HomeScreenDefault: React.FC<HomeScreenDefaultProps> = ({ onNavigate }) => {
    ...
    if (error || !data) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-t-bg p-4 transition-colors duration-300">
                <div className="bg-t-bg-elevated p-6 rounded-2xl shadow-sm border border-t-border text-center max-w-sm w-full transition-colors">
                    ...
                    <button
                        ...
                        className="w-full bg-t-accent text-white py-3 rounded-xl font-bold active:scale-[0.98] transition-transform hover:opacity-90"
                    >
                        Повторить
                    </button>
                </div>
    ...
    return (
        <div className="pb-24 min-h-screen relative bg-t-bg transition-colors duration-300 text-t-text">
            ...
            {/* Header */}
            <div className={`px-6 pt-12 pb-4 sticky ${offline ? 'top-8' : 'top-0'} z-20 bg-t-bg/80 backdrop-blur-xl border-b border-t-border/50 transition-colors`}>
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                        ...
                        className="relative p-2.5 bg-t-bg-elevated rounded-full hover:bg-t-bg-surface transition-colors border border-t-border"
                    >
                        <Bell className="w-5 h-5 text-t-text-secondary" />
                        {data.unreadNotifications > 0 && (
                            <span className="absolute top-2 right-2.5 w-2 h-2 bg-t-accent rounded-full shadow-[0_0_10px_var(--t-accent)]"></span>
                        )}
                    </button>
                </div>
                ...
                <div className="flex gap-4 overflow-x-auto no-scrollbar pb-1 -mx-6 px-6">
                    ...
                    <div className="flex flex-col items-center gap-2 cursor-pointer shrink-0">
                        <div className="w-16 h-16 rounded-full border border-dashed border-t-border p-1 flex items-center justify-center hover:border-t-accent transition-colors">
                            <div className="w-full h-full bg-t-bg-elevated rounded-full flex items-center justify-center text-t-accent">
                    ...
                    {data.stories.map((story) => (
                        ...
                        >
                            <div className={`w-16 h-16 rounded-full border-2 p-0.5 group-active:scale-95 transition-all ${story.isViewed ? 'border-t-border opacity-50' : 'border-t-accent shadow-[0_0_15px_var(--t-accent-glow)]'}`}>
                                <img
                                    src={story.imageUrl}
            ...
            <div className="p-5 space-y-8">
                ...
                <section className="grid grid-cols-2 gap-3">
                    ...
                    <button
                        ...
                        className="p-4 rounded-3xl flex items-center gap-3 transition-all group shadow-sm bg-t-bg-elevated/50 backdrop-blur-md border border-t-border hover:bg-t-bg-surface"
                    >
                        <div
                    ...
                    <button
                        ...
                        className={`p-4 rounded-3xl flex items-center gap-3 transition-all group shadow-sm backdrop-blur-md border hover:bg-t-bg-surface ${data.wallet.hasDebt
                                ? 'bg-t-error/10 border-t-error/30' // Red background for debt
                                : 'bg-t-bg-elevated/50 border-t-border'
                            }`}
                    >
                        <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform ${data.wallet.hasDebt ? 'bg-t-error/20' : 'bg-t-success/10'
                                }`}
                        >
                ...
                <section>
                    <div className="flex justify-between items-end mb-4 px-1">
                        ...
                        <button
                            ...
                            className="text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider transition-colors text-t-accent bg-t-accent/10 border border-t-accent/20 hover:bg-t-accent/20"
                        >
                            Расписание
                        </button>
                    </div>
                    ...
                    {data.nextBooking ? (
                        <div
                            ...
                            className="p-6 rounded-[2rem] relative overflow-hidden active:scale-[0.99] transition-all cursor-pointer group shadow-xl hover:shadow-2xl bg-t-bg-elevated border border-t-border"
                        >
                            <div
                            ...
                            <div className="flex items-start gap-5 relative z-10">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex gap-2 mb-2">
                                        <span
                                            className="px-2 py-0.5 text-[10px] font-bold uppercase rounded-md tracking-wider bg-t-accent/10 border border-t-accent/20 text-t-accent"
                                        >
                                            {data.nextBooking.date}
                                        </span>
                                        <span
                                            className="px-2 py-0.5 text-[10px] font-bold uppercase rounded-md tracking-wider bg-t-bg-surface border border-t-border text-t-text-secondary"
                                        >
                                ...
                                <div
                                    className="self-center p-3 rounded-full transition-colors shrink-0 bg-t-bg border border-t-border text-t-text-muted group-hover:bg-t-accent group-hover:text-white group-hover:border-t-accent group-hover:shadow-[0_0_15px_var(--t-accent)]"
                                >
                        ...
                        <div className="bg-t-bg-elevated p-8 rounded-[2rem] border border-t-border border-dashed text-center">
                            ...
                            <button
                                ...
                                className="bg-t-text text-t-bg px-6 py-3 rounded-xl text-sm font-bold shadow-lg hover:opacity-90 transition-colors uppercase tracking-wider"
                            >
                                Записаться
                            </button>
                        </div>
                        ...
                        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 -mx-5 px-5">
                            {data.promos.map((promo) => (
                                <button
                                    ...
                                    className="min-w-[280px] h-44 rounded-3xl overflow-hidden relative active:scale-95 transition-transform shrink-0 border border-t-border shadow-lg group"
                                >
                                    <img
                                        src={promo.imageUrl}
