...
const ProfileScreen: React.FC<ProfileScreenProps> = ({ onNavigate }) => {
  const { isEmber, isDarkMode } = useTheme();
  ...
  return (
    ...
    >
      ...
      >
        {!isEmber && <div className="absolute inset-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-b-3xl border-b border-gray-100 dark:border-zinc-800 -z-10" />}
        {/* Ambience */}
        <div
          className="absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none"
          style={{ background: isEmber ? 'rgba(255, 77, 0, 0.05)' : 'rgba(6, 182, 212, 0.05)' }}
        ></div>
        ...
        <div className="flex items-center gap-5 relative z-10">
          <div className="relative group">
            ...
            >
              {!isEmber && <div className="absolute inset-0 bg-gray-100 dark:bg-zinc-800 rounded-full border border-gray-200 dark:border-zinc-700" />}
            ...
            >
              {!isEmber && <div className="absolute inset-0 bg-white dark:bg-zinc-800 rounded-full border border-gray-200 dark:border-zinc-700" />}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {!isEmber && <span className="bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-gray-500 dark:text-zinc-400">ID {user.id}</span>}
                {isEmber && `ID ${user.id}`}
              </span>
              <span
                className="px-2 py-0.5 text-[10px] rounded-md font-bold uppercase tracking-wider"
              ...
              >
                {!isEmber && <span className="bg-cyan-50 dark:bg-cyan-950/30 border border-cyan-100 dark:border-cyan-900 text-cyan-600 dark:text-cyan-400">{user.tariffStatus}</span>}
                {isEmber && user.tariffStatus}
              </span>
            </div>
          </div>
        </div>
