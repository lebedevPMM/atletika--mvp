...
const PlanScreen: React.FC<PlanScreenProps> = ({ onNavigate }) => {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: isEmber ? 'var(--brand-bg-primary)' : undefined }}
    >
      {!isEmber && <div className="absolute inset-0 bg-gray-50 dark:bg-zinc-950 -z-10" />}
      {/* Offline Banner */}
      {isOffline && (
      ...
      >
        {!isEmber && <div className="absolute inset-0 bg-white dark:bg-zinc-900 rounded-b-3xl border-b border-gray-200 dark:border-zinc-800 -z-10" />}
        <div className="flex justify-between items-start mb-6">
          <div>
            <span
              className={`inline-block px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md mb-2 ${!isEmber ? 'bg-cyan-100 dark:bg-cyan-950/30 border border-cyan-200 dark:border-cyan-900 text-cyan-600 dark:text-cyan-400' : ''}`}
              style={isEmber ? {
            ...
            <p
              ...
              style={{ color: isEmber ? 'var(--brand-text-secondary)' : undefined }}
            >
              {planData.subtitle}
            </p>
          </div>
          <div className="text-right">
            <p
              ...
              style={{ color: isEmber ? 'var(--brand-text-muted)' : undefined }}
            >
              Тренер
            </p>
            <p
        ...
        {/* Progress Bar */}
        <div className="mb-6">
          <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: isEmber ? 'var(--brand-text-secondary)' : undefined }}>
            <span>Прогресс (Неделя {planData.currentWeek}/{planData.totalWeeks})</span>
            <span style={{ color: isEmber ? 'var(--brand-accent)' : undefined }}>{planData.progress}%</span>
          </div>
          <div
            className="w-full rounded-full h-2 overflow-hidden relative"
            style={{
              background: isEmber ? 'rgba(255,255,255,0.05)' : undefined,
              border: isEmber ? '1px solid var(--brand-border)' : undefined
            }}
          >
            {!isEmber && <div className="absolute inset-0 bg-gray-100 dark:bg-zinc-950 rounded-full border border-gray-200 dark:border-zinc-800" />}
            <div
              className="h-full rounded-full transition-all duration-500 relative z-[1]"
              style={{
                width: `${planData.progress}%`,
                background: isEmber ? 'var(--brand-progress-gradient)' : 'var(--brand-accent)',
                boxShadow: isEmber ? 'var(--brand-accent-glow)' : '0 0 10px #06b6d4'
              }}
            ></div>
          </div>
        </div>
        ...
        >
          {!isEmber && <div className="absolute inset-0 bg-gray-100 dark:bg-zinc-950 rounded-lg border border-gray-200 dark:border-zinc-800 -z-10" />}
          <CalendarIcon className="w-3.5 h-3.5 mr-2" style={{ color: isEmber ? 'var(--brand-text-muted)' : undefined }} />
          {planData.dateRange}
        </div>
      </div>
